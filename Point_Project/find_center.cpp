...
int main(void)
{
	cv::Mat imgHSV = cv::imread("C:/Users/TaeWon/Documents/Visual Studio 2017/OpenCVTest/bolad_circle.jpg");
	IplImage copy;
	...
	if (m_seq != 0)
	{
		...
		for (CvSeq* c = m_approxDP_seq; c != NULL; c = c->h_next)         // 엣지의 링크드리스트를 순회하면서 각 엣지들에대해서 출력한다.
		{
			...
			if (c->total > 15)                                       // 외곽선을 이루는 점의 갯수가 이것보다 미만이면 잡음이라고 판단
			{
				...
				cvCircle(Img_rgb, cvPoint(cvRound(cX), cvRound(cY)), 15, CVX_YELLOW, CV_FILLED);   // 중심점 찍어주기
	...
	imwrite("C:/Users/TaeWon/Documents/Visual Studio 2017/OpenCVTest/center_circle.jpg", imgOriginal);
