...
cv::Mat find_points(cv::Mat _image, FILE *out, string Point_name)
{
	...
	if (m_seq != 0)
	{
		...
		for (CvSeq* c = m_approxDP_seq; c != NULL; c = c->h_next)         // 엣지의 링크드리스트를 순회하면서 각 엣지들에대해서 출력한다.
		{
			...
			if ((c->total >= 16) && (c->total <= 16))                                     // 외곽선을 이루는 점의 갯수가 이것보다 미만이면 잡음이라고 판단
			{
				...
				int x_order, y_order;
				double cX, cY, m00;
				double RcX, RcY, zX1, zY1, pX1, pY1, zX2, zY2, pX2, pY2, sX, sY;
				zX1 = 211451.19, zY1 = 479170.417;
				zX2 = 211451.329, zY2 = 479167.288;  //항공지표 좌표값

				pX1 = 1997, pY1 = 7231;       
				pX2 = 2200, pY2 = 4890;    //항공지표 픽셀값  가로/세로 
				...
				printf("SX = %.4f , SY= %.4f \n ", sX, sY);    //1픽셀당 좌표변화량
				printf("CX1 = %.4f , CY1= %.4f \n ", sX*(RcX - pX1), sY*(RcY - pY1));  //항공좌표점 기준 좌표값 변화량 
				printf("zX = %.4f , zY= %.4f \n\n\n ", zX1 + (sX*(RcX - pX1)), zY1 + (sY*(RcY - pY1)));  //중간점 좌표값

				fprintf(out, "%.4f,%.4f,%.0f,%.0f,%s \n", zY1 + (sY*(RcY - pY1)), zX1 + (sX*(RcX - pX1)), RcY, RcX, Point_name);  //좌표 입력  좌표 Y,X ,Pix_Y,Pix,X , 이름

				cvCircle(Img_rgb, cvPoint(cvRound(cX), cvRound(cY)), 15, CVX_WHITE, CV_FILLED);   // 중심점 찍어주기
	...
	imwrite("C:/Users/TaeWon/Documents/Visual Studio 2017/OpenCVTest/circle_center.jpg", imgOriginal);
	c = cvWaitKey(0);
